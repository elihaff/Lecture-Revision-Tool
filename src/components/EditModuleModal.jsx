...
export function EditModuleModal({ module, onClose, onSave }) {
  ...
  const [abbreviation, setAbbreviation] = useState(module.abbreviation)
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await onSave({ name, abbreviation, color })
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
      <div className="bg-surface rounded-2xl w-full max-w-md border border-divider shadow-xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-divider">
          <h2 className="text-xl font-semibold text-primary">Edit Module</h2>
          <button
            onClick={onClose}
